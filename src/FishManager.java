...
/*
   魚：
         排泄：飽食度非0%時隨時間每秒增加0.5%，100%時排泄，造成ph值上升0.02
                     ，排泄完歸零
...
public class FishManager
{
	...
	private static final int ONETIME_FEED = 5;
	...
	public FishManager(ArrayList<Fish> fishes, Background bg, BackgroundManager bgm)
	{
	...
	public void feed()
	{
		...
		{
			if (fishes.get(i).getSatiation() < Fish.MAX_SATIATION)
			{
				// 每條魚飽食度上升5
	 ...
	 * @param autoFeedBtn
	 */
	public void autoFeed(Boolean autoFeedBtn)
	{
		if (autoFeedBtn)
		{
			for (int i = 0; i < fishes.size(); i++)
	...
	/**
	 * 生命力：依年齡決定生命力最高值如下，若溫度不在適溫內、不在適合ph值中、乾淨度低於40%、含氧量低於4ppm
	 ...
	 * @param sec
	 */
	public void growingLife(int sec) // 時間
	{
		double var;
		if (sec % 1 == 0)	//每秒鐘
		{
